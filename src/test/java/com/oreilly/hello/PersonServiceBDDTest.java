...
public class PersonServiceBDDTest
{
  ...
  {
    assertThat(service.getLastNames(), containsInAnyOrder("Hopper", "Lovelace", "Goldberg", "Borg", "Liskov"));
  }
  ...
  {
    given(repository.save(any(Person.class))).willReturn(people.get(0), people.get(1), people.get(2), people.get(3),
        people.get(4));
    ...
    then(repository).should(times(5)).save(any(Person.class));
    then(repository).should(never()).delete(any(Person.class));
  }

  public void useAnswer()
  {
    given(repository.save(any(Person.class))).will(invocation -> invocation.getArgument(0));
  ...
  {
    given(repository.save(any(Person.class))).willThrow(RuntimeException.class);
